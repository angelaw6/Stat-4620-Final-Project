...
```{r}
ratio_vars <- sapply(train, function(x) all(x >= 0 & x <= 1, na.rm = TRUE))
names(ratio_vars[ratio_vars == TRUE])
names(ratio_vars[ratio_vars == FALSE])
...
* Liquidity, Leverage, Turnover, and Cash Flow.
...
* ROA A/B/C before interest and after tax
  (these variables have high, near 1.0 correlation so either one of these variables could be selected)  
...
```
The response variable Bankruptcy is a binary variable that takes values of 0 or 1 to indicate if a company is or is not bankrupt. The numerical summary of the response variable in the training dataset shows that 97.07% of the dataset is 0s while only 2.93% of the dataset is 1s. This indicates that the dataset is very sparse and impalanced. This piece of information is important to note when thinking about potential modeling methods. 

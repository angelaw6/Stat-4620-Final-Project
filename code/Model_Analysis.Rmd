...
## Compare Ridge, LASSO, Bagging, and Random Forest
```{r}
...
ggplot(coef_df_top, aes(x = reorder(Variable, Coefficient), y = Coefficient)) +
  ...
  coord_flip() +
  labs(title = "Top 20 Ridge Coefficients (by Magnitude)",
       x = "Predictor",
       y = "Coefficient") +
  theme_minimal()
...
ggplot(ridge_prob_df, aes(x = Predicted_Prob, fill = Actual)) +
  ...
  labs(title = "Ridge Predicted Probability Distribution",
       x = "Predicted Bankruptcy Probability",
       fill = "Actual Class") +
  theme_minimal()

ridge_roc <- roc(y_test_factor, ridge_prob)
plot(ridge_roc, main = "ROC Curve - Ridge Regression")
auc(ridge_roc)

ridge_pred <- factor(ifelse(ridge_prob > 0.5, "1", "0"), levels = c("0","1"))

conf_mat_ridge <- confusionMatrix(ridge_pred, y_test_factor, positive = "1")
conf_mat_ridge
...
```{r}
# Ridge Clibration Curve
library(dplyr)
library(ggplot2)

cal_data <- data.frame(
  prob = as.numeric(ridge_prob),
  actual = as.numeric(as.character(y_test_factor))
)
...
ggplot(cal_summary, aes(x = mean_pred, y = actual_rate)) +
  ...
  labs(
    title = "Calibration Curve: Ridge Regression",
...
ggplot(lift_df, aes(x = depth, y = lift)) +
  ...
  geom_point(size = 2) +
  labs(
    title = "Lift Chart: Ridge Regression",
